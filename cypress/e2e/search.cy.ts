describe('Search and Sort', () => {
    ...
    it('Search files by name', () => {
        ...
        cy.wait(2000);
        cy.get('li p.subtitle-2.line-clamp-1')
            .should('contain.text', imageFile1);
    });
    ...
    it('Sort files by A-Z', () => {
        ...
        cy.get('section.file-list')
            ...
            .then(($els) => {
                const names = $els.toArray().map((el) => (el.textContent || '').trim().toLowerCase());
    ...
    it('Empty result', () => {
        ...
        cy.wait(2000);
        cy.get('ul.search-result p.empty-result').should('contain.text', 'No files found');
    });
