...
describe('Dashboard', () => {
  ...
  it('Signin existing user and logout', () => {
    cy.visit('/sign-in');
    cy.get('input[placeholder="Enter your email"]').type('test@example.com');
    cy.get('button[type="submit"]').click();
    ...
    cy.get('section.dashboard-recent-files').should('exist');
    cy.get('section.dashboard-recent-files h2').should('have.text', 'Recent files uploaded');

    cy.get('.chart-title').should('contain.text', 'Available Storage');
    cy.get('.chart-description').should('contain.text', '2GB');

    cy.get('.chart-details .chart-description').invoke('text').then((text) => {
    ...
    cy.get('.dashboard-summary-card').eq(0).find('p.body-1.text-light-200.text-center').invoke('text').should('match', /\d{1,2}:\d{2}(am|pm)/i);
    cy.get('.dashboard-summary-card').eq(1).find('p.body-1.text-light-200.text-center').invoke('text').should('match', /\d{1,2}:\d{2}(am|pm)/i);
    ...
    cy.get('button[type=submit]').click();
    cy.url().should('eq', 'http://localhost:3000/sign-in', { timeout: 10000 });
  });

  it('Duplicate email signup', () => {
    cy.visit('/sign-up');
    cy.get('input[placeholder="Enter your full name"]').type('Test User');
    cy.get('input[placeholder="Enter your email"]').type('test@example.com');
    cy.get('button[type="submit"]').click();
    
    cy.contains('p.shad-form-message', 'Email already in use', { timeout: 10000 })
      .should('be.visible');
  });
  ...
  it('No recent uploads', () => {
    cy.visit('/sign-up');
    cy.get('input[placeholder="Enter your full name"]').type('Test User');
    cy.get('input[placeholder="Enter your email"]').type('test123@example.com');
    cy.get('button[type="submit"]').click();
